function extracted_columns = ...
    medical_ambiguity_mean_choice_for_level(medical_choice, ambig_level, IGNORE_SLIGHT_IMPROVEMENT_COLUMN, is_verbose)
...
prefix = ['Med_Ambig_' num2str(ambig_level)]; % e.g. 'Med_Ambig_24'

% Get the logical index for columns that start with the specified prefix
col_idx = startsWith(medical_choice.Properties.VariableNames, prefix);

% Extract the columns with the specified prefix
extracted_columns = medical_choice(:, col_idx);

prefix_to_remove = [prefix '_SI']; % e.g. 'Med_Ambig_24_SI'
